...
class NPC(block):
	def __init__(self):
		block.__init__(self, textures = enemy)
...
class GameScreen():
	...
	def camera(self):
		...
		for b in self.left_up_for_bg:
			self.rect_bg.x =self.start_x= b.rect.x
			self.rect_bg.y =self.start_y= b.rect.y

	def remap(self):
		for c in self.allBlocks:
			c.rect.x -= self.start_x
			c.rect.y -= self.start_y + 20
	...
	def update(self):
		...
		self.NPC.update(self.hardBlocks)
		self.camera()
	...
	def draw(self):
		...
		pos = str(self.start_x) + " " + str(self.start_y)
