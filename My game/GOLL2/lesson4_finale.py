...
class player(pygame.sprite.Sprite):
	def __init__(self, coords = (0, 0)):
		...
		self.image = pygame.transform.scale(self.image,(tile_size, 80)).convert()
...
###
		...
		for block in killerBlocks:
			if pygame.sprite.collide_rect(self, block):
				self.hp -= 1
				self.respawn(self.starting_point)
...
class GameScreen():
	...
	def load_map(self):
		...
		for i, line in enumerate(file): # y
			for j, letter in enumerate(line): # x
				if letter == '1':
					block_temp = block(j*tile_size, i*tile_size, platforms) 
				...
				if letter == "*":
					block_temp = block(j*tile_size, i*tile_size, thorns)
					block_temp.add(self.allBlocks)
	...
	def camera_update(self): 
		for a in self.craiL:
			...
			if a.rect.y <= height-tile_size and self.player.rect.y > height/2:
				self.camera_vert = False
			else:
				self.camera_vert = True
		for b in self.center:
